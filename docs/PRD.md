...
**Changelog:** v1.1 — NFRs restructured; hard latency targets replaced with observability contract; scale ceiling reframed as design optimisation point, not hard limit.
...
KBWeaver builds directly on the *LLM Wiki* concept published by Andrej Karpathy in April 2026. Karpathy's core insight — that an LLM should act as a compiler, building a persistent structured wiki from raw sources rather than rediscovering knowledge from scratch on every query — is the intellectual foundation of this project.

His published concept is intentionally minimal: an idea specification without ingestion automation, entity resolution, a queryable graph layer, or a maintenance agent. KBWeaver's scope is defined by precisely those gaps.
...
- Processing time is reported in the ingestion report so the user can assess whether a component swap is warranted for their hardware.

---

### 3.2 Autonomous Knowledge Organisation
...
- Query latency is reported per run so the user can assess whether the current LLM backend is adequate.
...
2. The system reports:
   ...
   - Overall health metrics (note count, connection density, isolated clusters)
...
NFRs in this project fall into two distinct categories with different statuses. They must not be conflated.

### 4.1 Invariants (Hard Contracts)

These hold unconditionally. They are not subject to hardware variation or scale, and violating them breaks the product's core premise.

| Invariant               | Specification                                                                                                                                                        |
| ----------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| Data locality           | All processing is local by default. Cloud LLM backends are opt-in and require explicit user configuration. No data leaves the machine in the default configuration.  |
| Source preservation     | Original files dropped into the input folder are never modified. Originals are archived verbatim after processing.                                                   |
| Markdown validity       | All generated and updated notes must be valid, self-contained Markdown files, readable in Obsidian, Logseq, or any standard Markdown editor with no conversion step. |
| Rebuildability          | Deleting the entire `db/` directory and running `kbweaver rebuild` must return the system to a fully functional state from `wiki/` alone.                            |
| Supported input formats | PDF, DOCX, TXT, HTML, common code file extensions. This is a scope boundary, not a performance target.                                                               |

### 4.2 Operational Expectations (Observability Contract)

These are design targets for the initial hardware configuration, not hard constraints. The system must expose timing and health metrics at each subsystem boundary so the user can identify bottlenecks and make informed component swap decisions.

| Concern              | Design target                                       | Upgrade path                                                                                                                                            |
| -------------------- | --------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------- |
...
*For technical and architectural exclusions (e.g., vector embeddings, Windows support), see [ARCHITECTURE.md](./ARCHITECTURE.md).*